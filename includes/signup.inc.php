...
if (isset($_POST['signup-submit'])){
  ...
  if (empty($fname) || empty($lname) || empty($password) || empty($passwordRepeat) || empty($phoneno)|| empty($email)) {
  ...
  elseif ($password !== $passwordRepeat) {
    ...
    if(!mysqli_stmt_prepare($stmt, $sql)){
      ...
      if ($resultCheck > 0) {
        ...
        $sql ="INSERT INTO user(email, fname, lname, upassword, Phone_no) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt, $sql)){
          ...
          mysqli_stmt_bind_param($stmt, "ssssi", $email, $fname, $lname, $hashedPwd, $phoneno);
          mysqli_stmt_execute($stmt);
